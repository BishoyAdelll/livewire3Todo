<div wire:key={{$todo->id}} class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center ">
            @if($todo->completed)
            <input class="mr-2" wire:click="toggle({{$todo->id}})" type="checkbox" name="" id="" checked>
            @else
            <input class="mr-2" wire:click="toggle({{$todo->id}})" type="checkbox" name="" id="">
            @endif
    ...
    @if($editingTodoId === $todo->id)
        <button wire:click="updateTodo" class="mt-3 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600">Update</button>
